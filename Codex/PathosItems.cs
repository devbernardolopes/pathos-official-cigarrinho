          ...
          : base(Codex.Manifest.Items)
        {
            ...
            Item AddItem(Stock Stock, ItemType ItemType, string Name, Action<ItemEditor> DeclareAction)
            {
                return Register.Add(I =>
                {
                    ...
                    //formatter.Serialize(mstream, DeclareAction);
                    //mstream.Position = 0;
                    //byte[] bytes = mstream.GetBuffer();

                    //using (var stream = File.Create(@"C:\Games\Pathos\serialized.action.txt"))
                    //{
            ...
            {
                Item ii = AddItem(Stocks.scroll, ItemType.Scroll, Name, I =>
                {
                    DeclareAction(I);
                });

                //if (ii.HasUse(Motions.read))
                //{
                //    ii.
                //}
              ...
              T.WhenProbability(Table =>
        {
                ...
                I.AddObviousIngestUse(Motions.eat, DragonArmourNutrition, DragonArmourEatDelay, Sonics.armour, A =>
          {
            ...
            void AssignBookSpell(ItemEditor Item, Spell Spell, bool Fireproof = false)
            {
                ...
                Item.AddObviousUse(Motions.clairvoir, new Utility(Purpose.LearnSpell), Delay.FromTurns(Spell.Level * 10), Sonics.magic,
                    A => A.Apply.WhenSourceHasProperty(Properties.clairvoyance,
                        B =>
                        {
        ...
        (
          ...
          C => C.Heal(5.d4(), Modifier.Zero)
            ...
            wand_of_light = AddWand("wand of light", I =>
            {
                ...
                I.AddObviousUse(Motions.zap, WandZapDelay, Sonics.magic, Use =>
                {
                    Use.SetCast().Strike(Strikes.flash, Dice.Zero).SetTerminates().SetTargetSelf();
                    ...
                    Use.Apply.WithSourceSanctity
                    (
                        B => B.WhenTargetKind(Kinds.Extradimensional, A => A.WhenSourceBelowAppetite(Appetites.content, A => A.Nutrition(1.d20()))),
                        U => U.WhenTargetKind(Kinds.Extradimensional, A => A.WhenSourceBelowAppetite(Appetites.content, A => A.Nutrition(1.d10()))),
                        C => C.WhenTargetKind(Kinds.Extradimensional, A => A.DrainMana(Elements.drain, 2.d4()))
                    );
