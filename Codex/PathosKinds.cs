...
namespace Pathos
{
    public sealed class CodexKinds : CodexPage<ManifestKinds, KindEditor, Kind>
    {
          ...
          : base(Codex.Manifest.Kinds)
        {
            ...
            Kind AddExtradimensionalKind(string Name)
            {
                var kindExtended = Register.Add(K =>
                {
                    K.Name = Name;
                });

                kindExtended.SetExtradimensional(true);
            ...
            extradimensional = AddExtradimensionalKind("extradimensional");

            this.UndeadArray = List.Where(K => K.Undead).ToArray();
            this.LivingArray = List.Where(K => K.Living).ToArray();
            this.ExtradimensionalArray = List.Where(K => K. GetExtradimensional()).ToArray();

            //int? eaccc = this.ExtradimensionalArray?.Count();
        }
#endif

        public IReadOnlyList<Kind> Undead => UndeadArray;
        public IReadOnlyList<Kind> Living => LivingArray;
        public IReadOnlyList<Kind> Extradimensional => ExtradimensionalArray;
        ...
        public readonly Kind extradimensional;

        private readonly Kind[] UndeadArray;
        private readonly Kind[] LivingArray;
        private readonly Kind[] ExtradimensionalArray;
    }
}